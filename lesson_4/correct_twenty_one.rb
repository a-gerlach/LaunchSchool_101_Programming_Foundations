...
def win_or_lose(score, string, cards)
  if score > 21
    ...
    puts "The #{string} goes bust and looses this game."
    puts "Your hand in the end was: #{cards}"
    true
...
# first draw, calculate the score and add the card to the hand (array) of the player.
draw = draw_card(DCK)
...
draw = draw_card(DCK)
player_total_score += calculate_values!(draw, "player")
player_total_score = check_for_aces_v2(player_total_score, draw)

add_cards_to_hand(player_hand, draw)

win_or_lose(player_total_score, "player", player_hand)
...
# first draw, calculate the score and add the card to the hand (array) of the computer.
draw = draw_card(DCK)
...
win_or_lose(computer_hidden_score, "dealer", computer_hidden_card)
# puts "This is the hidden card: #{computer_hidden_card} and this is the real score with the hidden card: #{computer_hidden_score}"
...
loop do
  ...
  loop do
    ...
    if player_stay == false
      ...
      #player_total_score = check_for_aces_v2(player_total_score, draw) # this works, but want to try out the new version of check for aces      
    ...
    if computer_hidden_score < 17
      ...
      break if win_or_lose(computer_hidden_score, "dealer", computer_hidden_card)
    ...
    if player_stay && computer_stay
      compare_scores(player_total_score, computer_hidden_score)
      break
    end
...
# things to work on
# Does the check for aces work with the computer, i think so but have not checked definately
# be able to start a new game with two cards drawn
# be able to stay on the first turn
# 